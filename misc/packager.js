...
const basefile = path.join(__dirname, '..', 'code', 'track.h')
...
const versionGet = () => {
    ...
    const major = data.match(/TRACK_VERSION_MAJOR\s+([0-9]+)\n/)[1]
    const minor = data.match(/TRACK_VERSION_MINOR\s+([0-9]+)\n/)[1]
    const patch = data.match(/TRACK_VERSION_PATCH\s+([0-9]+)\n/)[1]
    const pre   = data.match(/TRACK_VERSION_PRE\s+\"([\.a-z0-9]+)\"\n/)
...
const versionSet = (version) => {
    ...
    data = data.replace(/TRACK_VERSION_MAJOR\s+([0-9]+)\n/, `TRACK_VERSION_MAJOR ${major}\n`)
    data = data.replace(/TRACK_VERSION_MINOR\s+([0-9]+)\n/, `TRACK_VERSION_MINOR ${minor}\n`)
    data = data.replace(/TRACK_VERSION_PATCH\s+([0-9]+)\n/, `TRACK_VERSION_PATCH ${patch}\n`)
    data = data.replace(/TRACK_VERSION_PRE\s+\"([\.0-9a-z]+)\"\n/, `TRACK_VERSION_PRE "${pre}"\n`)
...
const embedIncludes = (print) => {
    ...
    else       fs.writeFileSync(path.join(workdir, 'track.h'), code)
}
...
class Bumper extends Plugin {
    ...
    afterRelease() {
        if (fs.existsSync(path.join(workdir, 'track.h'))) {
            fs.unlinkSync(path.join(workdir, 'track.h'))
        }
    }
}
